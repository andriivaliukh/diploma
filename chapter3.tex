...
\caption{Вивід wg show: активний тунель, handshake та transfer counter}
\label{fig:func-07}
\end{figure}
...
\textbf{Сценарій} & \textbf{Медіана ± СКВ} & \textbf{p95} & \textbf{Примітка} \\\hline
...
каналу. OpenVPN, навпаки, показує мінімальний bufferbloat (0{,}48~мс) попри
...
\begin{tabularx}{\textwidth}{|l|X|X|X|}\hline
\textbf{Сценарій} & \textbf{p95} & \textbf{Буферний bloat, мс} & \textbf{Примітка} \\\hline
Без VPN (базовий)     & $24{,}20$ & $+0{,}12$ & еталон \\\hline
WG plain (ядро)       & $26{,}60$ & $+2{,}40$ & sk\_buff queue fill \\\hline
Ця система (WG+2FA)   & $27{,}26$ & $+3{,}06$ & network ns hop \\\hline
...
\textbf{Сценарій}
  & \textbf{tcp\_t (1 потік)}
  & \textbf{tcp\_p (4 потоки)}
  & \textbf{$\Delta_{p-t}$,\,\%}
  & \textbf{UDP (перший крок втрат)} \\\hline
...
лише 18--19~Мбіт/с (табл. в розд.~3 агрегованих даних), а
...
по собі є архітектурним висновком цього дослідження~\cite{mdpi2025wgopenvpn,os3_2020_vpn}.
...
\begin{tabularx}{\textwidth}{|l|X|X|}\hline
\textbf{Сценарій} & \textbf{Медіана} & \textbf{Примітка} \\\hline
...
даних \cite{mdpi2025wgopenvpn,os3_2020_vpn}. WireGuard перевищує OpenVPN за
...
userspace daemon, pidstat) вимірюються різними інструментами та
