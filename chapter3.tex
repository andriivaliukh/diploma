...
\caption{Відповідь реєстрації користувача (HTTP 201) з полями totp\_secret та qr\_code, а також підтвердження TOTP-реєстрації (HTTP 200) з success=true}
\label{fig:func-02}
\end{figure}
...
тунель (\textit{onboarding}) відповідає визначенню з розділу~3.6
(час від початку команди до отримання першої ICMP-відповіді, з попередньо
завантаженим токеном). Проміжки між повтореннями~--- не менше 10 с для
уникнення теплових ефектів.
...
сервера фіксувалось командою \texttt{mpstat} пакету sysstat \cite{mpstat_sysstat}.
...
пропускна здатність (~55~Мбіт/с) означає, що черга шифрувального потоку
...
\begin{table}[htbp]\centering
\caption{Пропускна здатність TCP та UDP (Мбіт/с), N=5, iperf3 60~с}
\label{tab:perf_tcp}
...
ефективно задіює механізми TSO/GSO \cite{donenfeld2017wireguard}, зберігаючи
великі сегменти до точки передачі до мережевого стека~--- додаткові потоки
майже не дають приросту. Для OpenVPN та сама різниця становить
\textbf{36{,}3\,\%}. Один потік обмежений серіалізацією через єдиний
шифрувальний потік \texttt{openvpn} (~55~Мбіт/с). Чотири паралельні потоки
...
заповненою та збільшує пропускну здатність до ~74~Мбіт/с. Тим не менше
...
Ця система (WG+2FA)   & 1571  & HTTP POST /register → POST /login (TOTP) → key reg → \texttt{wg-quick up} \\\hline
...
потоці трафіку не виникає. Таким чином, 1{,}5~с сплачуються одноразово при
кожному відновленні сесії й дають натомість перевіряємий ідентифікатор
...
перемикань контексту; OpenVPN's однопотоковий daemon AES-256-CBC обмежений
