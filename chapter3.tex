...
статичного ключа з шифруванням AES-256-GCM, UDP-порт~1194
...
\label{subsec:perf_results}

\begin{table}[htbp]\centering
\caption{Затримка (мс) без навантаження, N=5}
\label{tab:perf_lat_idle}
\begin{tabularx}{\textwidth}{|l|X|X|X|X|}\hline
\textbf{Сценарій} & \textbf{Середнє ± СКВ} & \textbf{Медіана} & \textbf{p95} & \textbf{Примітка} \\\hline
Plain WG (ядро)       & \TODO{} & \TODO{} & \TODO{} & \\\hline
Ця система            & \TODO{} & \TODO{} & \TODO{} & \\\hline
OpenVPN (AES-256-GCM) & \TODO{} & \TODO{} & \TODO{} & \\\hline
\end{tabularx}\end{table}

\begin{table}[htbp]\centering
\caption{Затримка (мс) під навантаженням, N=5}
\label{tab:perf_lat_load}
\begin{tabularx}{\textwidth}{|l|X|X|X|X|}\hline
\textbf{Сценарій} & \textbf{Середнє ± СКВ} & \textbf{Медіана} & \textbf{p95} & \textbf{Буферний bloat} \\\hline
Plain WG (ядро)       & \TODO{} & \TODO{} & \TODO{} & \TODO{} \\\hline
Ця система            & \TODO{} & \TODO{} & \TODO{} & \TODO{} \\\hline
OpenVPN (AES-256-GCM) & \TODO{} & \TODO{} & \TODO{} & \TODO{} \\\hline
\end{tabularx}\end{table}

\begin{table}[htbp]\centering
\caption{Пропускна здатність TCP (Мбіт/с), N=5, iperf3 60~с}
\label{tab:perf_tcp_t}
\begin{tabularx}{\textwidth}{|l|X|X|X|X|}\hline
\textbf{Сценарій} & \textbf{Одиночний потік} & \textbf{4 потоки (SUM)} & \textbf{СКВ} & \textbf{Примітка} \\\hline
Plain WG (ядро)       & \TODO{} & \TODO{} & \TODO{} & \\\hline
Ця система            & \TODO{} & \TODO{} & \TODO{} & \\\hline
OpenVPN (AES-256-GCM) & \TODO{} & \TODO{} & \TODO{} & \\\hline
\end{tabularx}\end{table}
...
\begin{table}[htbp]\centering
\caption{Стійка UDP-пропускна здатність (Мбіт/с), N=5}
\label{tab:perf_udp}
\begin{tabularx}{\textwidth}{|l|X|X|}\hline
\textbf{Сценарій} & \textbf{Максимальна швидкість (втрати < 1\,\%)} & \textbf{Примітка} \\\hline
Plain WG (ядро)       & \TODO{} & \\\hline
Ця система            & \TODO{} & \\\hline
OpenVPN (AES-256-GCM) & \TODO{} & \\\hline
\end{tabularx}\end{table}
...
\textbf{Сценарій} & \textbf{Середній \%soft (WG/ця система)} & \textbf{Середній \%CPU (OpenVPN)} & \textbf{Примітка} \\\hline
Plain WG (ядро) & \TODO{} & --- & mpstat \\\hline
Ця система      & \TODO{} & --- & mpstat \\\hline
OpenVPN (AES-256-GCM) & --- & \TODO{} & pidstat \\\hline
\end{tabularx}\end{table}
...
\begin{tabularx}{\textwidth}{|l|X|X|X|}\hline
\textbf{Сценарій} & \textbf{Середнє ± СКВ} & \textbf{Медіана} & \textbf{Примітка} \\\hline
Plain WG (ядро)       & \TODO{} & \TODO{} & \texttt{wg-quick up} + ping-цикл \\\hline
Ця система            & \TODO{} & \TODO{} & \texttt{vpncli connect -{}-emit-timings} \\\hline
OpenVPN (AES-256-GCM) & \TODO{} & \TODO{} & \texttt{openvpn -{}-daemon} + ping-цикл \\\hline
\end{tabularx}\end{table}

\subsection{Обговорення}
\label{subsec:perf_discussion}

Ця система побудована поверх ядерного WireGuard і не вносить власного
шифрувального рівня в канал даних. Тому очікується, що показники затримки
та пропускної здатності TCP будуть статистично еквівалентними показникам
чистого WireGuard (у межах шуму апаратного стенду), що узгоджується з
висновками~\cite{mdpi2025wgopenvpn,donenfeld2017wireguard}.

Превага протоколу WireGuard у пропускній здатності над OpenVPN пояснюється
ядерною реалізацією (менше копіювань між просторами ядра та користувача),
підтримкою GSO/TSO починаючи з ядра~6.2, а також алгоритмом
ChaCha20-Poly1305 із апаратним прискоренням на процесорах ARM та x86 (CLMUL).
OpenVPN у режимі статичного ключа з AES-256-GCM уникає TLS-накладних витрат
на пакет, тому результати потрібно розглядати як верхню межу для OpenVPN
(реальне PKI-розгортання додає накладні витрати на рукостискання).

Показник онбордингу розробленої системи включає цикл HTTP-автентифікації
(пароль + TOTP → JWT) та створення WireGuard-сесії через API, що додає
кілька сотень мілісекунд до часу \texttt{wg-quick up}; точні значення
наведено у табл.~\ref{tab:perf_onboard}.
...
\begin{enumerate}
  ...
  \item \textbf{Асиметрія крипто-наборів.} WireGuard та ця система
        ...
        AES-256-GCM у режимі статичного ключа; IPsec (якщо вимірюється)~---
        AES-256-GCM-IKEv2 \cite{rfc7296}. Пряме порівняння числових
        результатів можливе лише за рівних умов.
  ...
  \item \textbf{Режим OpenVPN.} OpenVPN вимірюється у режимі статичного
        ключа (deprecated у OpenVPN~2.6, але стабільний). PKI/TLS-режим
        потенційно демонструє нижчу пропускну здатність через додаткові
        TLS-накладні витрати на пакет.

  \item \textbf{Версія ядра та ОС.} Стенд використовує Ubuntu~26.04
        (ядро~7.0) на обох вузлах. Результати можуть відрізнятись на
        старих ядрах, зокрема до~6.2, де відсутня підтримка WireGuard
        GSO/TSO.

  \item \textbf{Область охоплення клієнтського ОС (rev~4).} Показники
        ...
        тестовано. Результати характеризують Linux-шлях клієнта;
        крос-платформна валідація є темою майбутніх досліджень.
\end{enumerate}
...
(очікувана паритетність у межах~\TODO{}\,\%), оскільки обидва рішення
використовують один і той самий ядерний тунельний модуль на платформі
Ubuntu~26.04~LTS. Накладні витрати HTTP-автентифікації додають~\TODO{}~мс
до часу введення у тунель порівняно з~\TODO{}~мс для чистого
\texttt{wg-quick up}.
