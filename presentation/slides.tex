...
\begin{frame}[plain]
  ...
  {\normalsize\color{clInk}
    \begin{tabular}{@{}r@{\;—\;}l@{}}
      Виконав           & Валюх А.А.  \\[0.08em]
      Науковий керівник & Бойко Ю.В.  \\
    \end{tabular}
  }
...
%% ══════════════════════════════════════════════════════════════════════════════
\begin{frame}[c]
  \frametitle{Як працює}
  \fitfig{fig-sequence}
\end{frame}


%% ══════════════════════════════════════════════════════════════════════════════
%%  SLIDE 6 — Результати                                    (48 с)
%%  One idea: data plane = parity with plain WireGuard; 2FA cost is one-time.
%%  Cycle: Rep. 3 — «тому й вимірюється нуль: протокол не змінено».
%%  Numbers + anchor + callout «одноразово, не постійно» are baked into fig-results.tex.
%% ══════════════════════════════════════════════════════════════════════════════
\begin{frame}[c]
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item \textbf{Запропоновано} модель «сесія як вузол» ---
          2FA без зміни протоколу і без PSK.
    \item \textbf{Реалізовано} повний стек (сервер + CLI);
          149 автотестів, 7 функціональних --- успішно.
    \item \textbf{Виміряно} паритет площини даних
          (1\,\%\,/\,0\,\%); одноразовий онбординг --- 1571\,мс.
    \item \textbf{Підготовлено} як відкрите self-hosted
          (стандартний WG-клієнт, без зовнішнього IdP).
  \end{itemize}
\end{frame}
...
%% ── R2: Дві площини — відкликання ────────────────────────────────────────────
\begin{frame}[c]
  \frametitle{Дві площини (резерв)}
  \fitfig{fig-planes}
\end{frame}
